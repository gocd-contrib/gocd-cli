...
class ConfigRepoCheck : CliktCommand(name = "check") {
  private val pluginsDir: File by option("-d", "--plugins-dir", help = "The path from which to load pluginsDir. Defaults to \"\$HOME/.gocd/plugins\"").file(exists = true, fileOkay = false, readable = true).default(File(configDir(), "plugins"))
...
private fun ensureDirExists(dir: File) {
  ...
  if (!dir.exists() || !dir.isDirectory) {
    echo("Failed to create $dir; be sure the parent directory exists and is writable", err = true)
    exitProcess(1)
  }
}
